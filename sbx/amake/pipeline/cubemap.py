...
class CubemapTool(CommonAssetTool):
    """
    ...
    def define_outputs(self, file_path: Path) -> List[Path]:
        outputs = [self.output_folder / file_path.relative_to(self.input_folder).with_suffix(".bin")]

        try:
            with file_path.open("r", encoding="utf-8") as f:
                data = json.load(f)
            if data["type"] == "equirectangular_cubemap":
                return outputs + [self.output_folder / file_path.relative_to(self.input_folder).parent / Path(data["output"][dv]) for dv in dirs]
            else:
                raise NotImplementedError("Haven't implemented that type of cubemap yet")
        except Exception as e:
            print(f"Cubemap Error reading {file_path}: {e}")
    ...
    def build(file_path: Path, input_folder: Path, output_folder: Path) -> None:
        ...
        bin_path = output_folder / file_path.relative_to(input_folder).with_suffix(".bin")
        ...
        def get_direction(face: str, u: np.ndarray, v: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
            """
        ...
        def generate_face_map(face: str, face_size: int, eq_width: int, eq_height: int) -> Tuple[np.ndarray, np.ndarray]:
            """
        ...
        for face in dirs:
            ...
            out_path = output_folder / file_path.relative_to(input_folder).parent / Path(rel_out)
