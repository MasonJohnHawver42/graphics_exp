...
class AtlasTool(CommonAssetTool):
    """
    ...
    */
    ...
    def define_outputs(self, file_path: Path) -> List[Path]:
        return [self.output_folder / file_path.resolve().relative_to(self.input_folder).with_suffix(".atlas.bin")]
    ...
    def build(file_path: Path, input_folder: Path, output_folder: Path) -> None:
        ...
        output_bin_file = output_folder / file_path.resolve().relative_to(input_folder).with_suffix(".atlas.bin")
