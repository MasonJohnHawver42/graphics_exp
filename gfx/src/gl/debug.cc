...
gl::debug::primitives::IRenderer::~IRenderer() = default;
...
gl::debug::primitives::ModelCacheCreateInfo::ModelCacheCreateInfo(const cm::MappedFile& cache_file, std::pmr::memory_resource* mr) :
    ModelCacheCreateInfo(cache_file.data(), cache_file.mappedSize(), mr) {}


std::ostream& operator<<(std::ostream& os, const gl::debug::primitives::ModelCacheCreateInfo& info) {
    // === Print Models ===
    os << "\n=== Models ===\n";
    os << std::setw(4)  << "Idx"
        << std::setw(16) << "Name"
...
gl::debug::primitives::ModelCache::ModelCache(const ModelCacheCreateInfo& params, std::pmr::memory_resource* mr)
...
gl::debug::primitives::ModelCache::~ModelCache() {
    glDeleteBuffers(1, &vbo);
    glDeleteBuffers(1, &ebo);
    glDeleteVertexArrays(1, &vao);
}

gl::debug::primitives::NaiveRenderer::NaiveRenderer(const ShaderProgramCreateInfo& shader_param, const ModelCacheCreateInfo& cache_param, std::pmr::memory_resource* mr) :
...
void gl::debug::primitives::NaiveRenderer::begin(const FrameCommand& cmd) {
    current_frame = cmd;
    draw_queue.clear();
}

void gl::debug::primitives::NaiveRenderer::submit(const DrawCommand& cmd) {
    draw_queue.push_back(cmd);
}

void gl::debug::primitives::NaiveRenderer::end() {
    program.use();
...
unsigned int gl::debug::primitives::NaiveRenderer::model_id(std::string&& query) {
...
gl::debug::quads::IRenderer::~IRenderer() = default;
...
unsigned int _QuadIndices[] = {
    0, 1, 2,
    2, 3, 0
};

gl::debug::quads::NaiveRenderer::NaiveRenderer(const ShaderProgramCreateInfo& shader_param, std::pmr::memory_resource* mr) :
    program(shader_param, mr), draw_queue(mr) 
{
    u_pos = program.getUniformLocation("uPosition", GL_FLOAT_VEC4);
    u_texCoords = program.getUniformLocation("uTexCoords", GL_FLOAT_VEC4);
    u_texture = program.getUniformLocation("uTexture", GL_SAMPLER_2D);
...
void gl::debug::quads::NaiveRenderer::begin() 
{
    draw_queue.clear();
}

void gl::debug::quads::NaiveRenderer::submit(const DrawCommand& cmd) 
{
    draw_queue.push_back(cmd);
}

void gl::debug::quads::NaiveRenderer::end() 
{
    program.use();
    glBindVertexArray(vao);
    ...
    for (const auto& draw : draw_queue) {
        ...
        glUniform4fv(u_pos, 1, &draw.pos.data[0]);
        glUniform4fv(u_texCoords, 1, &draw.texCoord.data[0]);
